...
window.computeAPEX = function(home, away, comp, hFormStats, aFormStats) {
  ...
  const hGFpg = hFormStats ? parseFloat(hFormStats.gfpg) : null;
  const aGFpg = aFormStats ? parseFloat(aFormStats.gfpg) : null;
  const hGApg = hFormStats ? parseFloat(hFormStats.gapg) : null;
  const aGApg = aFormStats ? parseFloat(aFormStats.gapg) : null;
  ...
  const hXG = hSOT > 0
    ...
    : hGFpg != null
      ? +(hGFpg * 0.82).toFixed(2)   // scale down slightly (xG < actual goals usually)
      : +(0.55 + hRecentWR*1.2 + hRecentDR*0.3).toFixed(2);
  const aXG = aSOT > 0
    ? +(aSOT*0.33 + aSh*0.04).toFixed(2)
    : aGFpg != null
      ? +(aGFpg * 0.82).toFixed(2)
      : +(0.45 + aRecentWR*1.1 + aRecentDR*0.25).toFixed(2);
  ...
  let hWin = espnProb != null
    ...
    : Math.round(Math.max(15, Math.min(75, (hWRForProb*52 + 13 + (hXG-aXG)*4))));
