...
window.pickPredictions = function(apex) {
  ...
  // Home line — pick Over 1.5 if strong enough, else Over 0.5
  if (hXG >= 1.4)
    teamGoalOpts.push({ ...ML.goals[3], id:'ho15', label:'Home Over 1.5',
      ...
      insight:`Home team xG of ${hXG} is high enough to expect 2+ goals from them` });
  else if (hXG >= 0.72)
    teamGoalOpts.push({ ...ML.goals[2], id:'ho05', label:'Home Over 0.5',
      prob: Math.round(Math.min(85, hXG*38+28)),
      reason:`Home xG ${hXG}`,
      insight:`Home xG of ${hXG} — highly likely to get on the scoresheet at least once` });

  // Away line — pick Over 1.5 if strong enough, else Over 0.5
  if (aXG >= 1.2)
    teamGoalOpts.push({ ...ML.goals[5], id:'ao15', label:'Away Over 1.5',
      ...
      insight:`Away xG of ${aXG} — they are expected to score more than once here` });
  else if (aXG >= 0.62)
    teamGoalOpts.push({ ...ML.goals[4], id:'ao05', label:'Away Over 0.5',
      prob: Math.round(Math.min(83, aXG*38+22)),
      reason:`Away xG ${aXG}`,
      insight:`Away xG of ${aXG} — enough quality to trouble the home defence` });

  // Pick the stronger of home/away line for this category
