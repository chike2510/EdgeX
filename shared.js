...
window.pickPredictions = function(apex) {
  ...
  if ((hW + aW) >= 68 && dr <= 22 && o15 >= 64) {
    ...
    comboOpts.push({ ...ML.combo[4], prob: p12,
      ...
      insight:`Both teams capable, draw unlikely (${dr}%) — 12 + goals is strong combined bet` });
  }
...
window.getAPEXAIInsight = async function(apex, homeName, awayName, competition) {
  const prompt = `You are EdgeX APEX, an elite sports betting analyst. Analyze this match and give a sharp, data-driven betting insight.
...
Provide a 3-sentence betting insight:
1. The single strongest market to target and why (cite the data)
...
Be sharp and concise. No filler. Max 120 words.`;
  ...
  try {
    const r = await fetch('/api/ai', {
      ...
      body: JSON.stringify({
        prompt,
        system: 'You are a sharp, concise sports betting analyst. Respond in plain text, no markdown, no bullet points. Direct and data-driven.',
        max_tokens: 300,
      }),
    });
...
window.scoreBayseEdge = function(modelProb, marketProb) {
  ...
  const direction = diff > 0 ? 'YES' : 'NO'; // which side to trade
  ...
  return {
    ...
    edge,
    direction,
    // Kelly fraction (simplified) — bet size guidance
    kelly: diff > 0 ? +((modelProb/100 - (1 - mp/100)) / (mp/100)).toFixed(3) : 0,
    verdict: edge === 'HIGH'
      ? `Strong edge — model vs market gap is ${absDiff.toFixed(0)}%. Trade ${direction}.`
      : edge === 'MEDIUM'
      ? `Moderate edge (${absDiff.toFixed(0)}%). Consider ${direction} if other factors align.`
      : `Thin edge (${absDiff.toFixed(0)}%). Skip or wait for better price.`,
  };
};
