...
window.computeAPEX = function(home, away, comp, hFormStats, aFormStats, hTeamStats, aTeamStats) {
  ...
  // hFormStats/aFormStats come from /api/form — wins, losses, draws, gfpg, gapg etc
  ...
  // Goals per game from recent form is the best xG proxy we have
  // Safe parse — guard against 'NaN' string or missing values
  ...
  // ── xG calculation ────────────────────────────────────────────
  // Priority: 1) live shot stats  2) recent goals/game  3) season win rate
  const hXGraw = hSOT > 0
    ? hSOT*0.33 + hSh*0.04
    : hGFpg != null
      ? hGFpg * 0.82
      : 0.55 + hRecentWR*1.2 + hRecentDR*0.3;
  const aXGraw = aSOT > 0
    ? aSOT*0.33 + aSh*0.04
    : aGFpg != null
      ? aGFpg * 0.82
      : 0.45 + aRecentWR*1.1 + aRecentDR*0.25;
  ...
  const hWRForProb = hFormStats ? hRecentWR : hWR;
  const aWRForProb = aFormStats ? aRecentWR : aWR;
...
window.pickPredictions = function(apex) {
  ...
  const hCS = apex.hCS || 0;
  const aCS = apex.aCS || 0;
  ...
  const goalOpts = [];
  // Over markets
  if (apex.o25 > 52) goalOpts.push({ ...ML.goals[0], id:'over25', label:'Over 2.5 Goals',
  ...
  if (apex.o15 > 65) goalOpts.push({ ...ML.goals[0], id:'over15', label:'Over 1.5 Goals',
    prob: apex.o15, reason:`xG ${totalXG.toFixed(2)} — 2+ goals likely`,
  ...
  const u25prob = Math.round(100 - apex.o25);
  const u15prob = Math.round(100 - apex.o15);
  ...
  // GG / No GG
  if (apex.btts > 55 && hXG > 0.72 && aXG > 0.62) goalOpts.push({ ...ML.goals[6],
    prob: apex.btts, reason:`Both teams xG: H${hXG} / A${aXG}`,
    insight:`Home xG ${hXG} and Away xG ${aXG} — both sides creating enough to score` });
  const nogGprob = Math.round(100 - apex.btts);
  ...
  // ── CATEGORY 3: TEAM GOALS (individual) ──────────────────────
  const teamGoalOpts = [];
  if (hXG >= 1.4) teamGoalOpts.push({ ...ML.goals[3], id:'ho15', label:'Home Over 1.5',
    prob: Math.round(Math.min(82, hXG*28+42)),
    reason:`Home xG ${hXG} — strong attacking output`,
    insight:`Home team averaging ${hXG} xG — quality enough to score 2+ in this fixture` });
  else if (hXG >= 0.82) teamGoalOpts.push({ ...ML.goals[2], id:'ho05', label:'Home Over 0.5',
    prob: Math.round(Math.min(85, hXG*38+28)),
    reason:`Home xG ${hXG}`,
    insight:`Home team xG of ${hXG} — high probability of scoring at least once` });
  if (aXG >= 1.2) teamGoalOpts.push({ ...ML.goals[5], id:'ao15', label:'Away Over 1.5',
    prob: Math.round(Math.min(80, aXG*28+40)),
    reason:`Away xG ${aXG} — genuine multi-goal threat`,
    insight:`Away team's xG of ${aXG} suggests they are likely to score more than once` });
  else if (aXG >= 0.70) teamGoalOpts.push({ ...ML.goals[4], id:'ao05', label:'Away Over 0.5',
    prob: Math.round(Math.min(83, aXG*38+22)),
    reason:`Away xG ${aXG}`,
    insight:`Away xG of ${aXG} — strong enough to trouble the home defence` });
  // 2nd half goal market
  if (totalXG > 1.5) teamGoalOpts.push({ ...ML.goals[1], id:'2h05', label:'2nd Half Over 0.5',
    prob: Math.round(Math.min(86, totalXG*18+52)),
    reason:`Total xG ${totalXG.toFixed(2)}`,
    insight:`With ${totalXG.toFixed(2)} combined xG, a 2nd half goal is highly probable` });
  ...
  const comboOpts = [];
  if (hW > 45 && apex.btts > 50) comboOpts.push({ ...ML.combo[0],
    prob: Math.round((hW*0.55 + apex.btts*0.45) + 4),
    reason:`Home ${hW}% + GG ${apex.btts}%`,
    insight:`Home win or both teams score — strong double cover for this fixture` });
  if (aW > 40 && apex.btts > 50) comboOpts.push({ ...ML.combo[1],
    prob: Math.round((aW*0.55 + apex.btts*0.45) + 3),
    reason:`Away ${aW}% + GG ${apex.btts}%`,
    insight:`Away win or both score — good cover in what should be an open game` });
  if (dr > 23 && apex.o25 > 48) comboOpts.push({ ...ML.goals[8],
    prob: Math.round(dr*0.5 + apex.o25*0.55 + 8),
    reason:`Draw ${dr}% + Over 2.5 ${apex.o25}%`,
    insight:`Even matchup with goals expected — Draw or Over 2.5 covers most outcomes` });
  if (hW > 48 && apex.o25 > 46) comboOpts.push({ ...ML.goals[9],
    prob: Math.round(hW*0.55 + apex.o25*0.45),
    reason:`Home ${hW}% or Over 2.5 ${apex.o25}%`,
    insight:`Home side favoured in what should be a goalsy game — excellent coverage` });
  if (aW > 44 && apex.o25 > 46) comboOpts.push({ ...ML.goals[10],
    prob: Math.round(aW*0.55 + apex.o25*0.45),
    reason:`Away ${aW}% or Over 2.5 ${apex.o25}%`,
    insight:`Strong away team in an open game — Away or Over 2.5 provides wide cover` });
  if ((hW > 52 || aW > 48) && apex.o15 > 63) comboOpts.push({ ...ML.goals[11],
    prob: Math.round(Math.max(hW,aW)*0.45 + apex.o15*0.55),
    reason:`Clear favourite + Over 1.5`,
    insight:`Dominant team in a game that should produce goals — Double Chance + Over 1.5` });
  ...
  if (totalXG < 1.55 && apex.o25 < 44) halvesOpts.push({ ...ML.halves[4],
    prob: Math.round(100 - apex.o25 - 8),
  ...
  // ── CATEGORY 6: CORNERS ───────────────────────────────────────
  const cornerOpts = [];
  // Use estimated corners + insight with per-team breakdown
  const cOver95prob = Math.round(Math.min(78, totalCornersEst*8 + 22));
  const cUnder95prob = Math.round(100 - cOver95prob);
  if (totalCornersEst >= 9.5) cornerOpts.push({ ...ML.corners[0], id:'co95',
    label:'Corners Over 9.5',
    prob: cOver95prob,
    reason:`Projected ${totalCornersEst} corners (H${hCornersEst} + A${aCornersEst})`,
    insight:`Corner projection: ${totalCornersEst} total — Home avg ${hCornersEst}, Away avg ${aCornersEst}. Over 9.5 supported.` });
  if (totalCornersEst < 9.5) cornerOpts.push({ ...ML.corners[1], id:'cu95',
    label:'Corners Under 9.5',
    prob: Math.min(76, cUnder95prob),
    reason:`Projected ${totalCornersEst} corners (H${hCornersEst} + A${aCornersEst})`,
    insight:`Low corner expectation at ${totalCornersEst} total — Home ${hCornersEst} avg and Away ${aCornersEst} avg aren't prolific corner winners` });
  // Over/Under 7.5
  const cOver75prob = Math.round(Math.min(82, totalCornersEst*7 + 28));
  if (totalCornersEst >= 7.5) cornerOpts.push({ ...ML.corners[0], id:'co75',
    label:'Corners Over 7.5',
    prob: cOver75prob,
    reason:`${totalCornersEst} projected corners`,
    insight:`At ${totalCornersEst} projected total, clearing 7.5 corners is highly probable` });
  cornerOpts.sort((a,b) => b.prob - a.prob);
  if (cornerOpts.length) cats['corners'] = { ...cornerOpts[0], cat:'🏁 Corners', ...risk(cornerOpts[0].prob) };
