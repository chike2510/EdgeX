...
export default async function handler(req, res) {
  ...
  try {
    ...
    const extractScore = (competitor, allCompetitors) => {
      ...
      // Try displayValue on competitor
      ...
      // Try winner flag — if one team is winner we know relative result
      // but can't get exact score this way
    ...
    const form = completed.map(ev => {
      ...
      let teamScore = extractScore(team, competitors);
      let oppScore  = extractScore(opp, competitors);

      // If scores unavailable, use winner flag to determine W/D/L
      let result;
