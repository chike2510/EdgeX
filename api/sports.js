  ...
  try {
    switch (type) {
      ...
      case 'football-live': {
        ...
        // Parallel: live + today + tomorrow in one shot
        const [liveResp, fd0, fd1] = await Promise.all([
          get(FB_BASE + '/fixtures', { live: 'all' }),
        ...
        const combined = [
          ...(fd0.response||[]).filter(f => FOOTBALL_LEAGUES.has(f.league?.id)),
          ...(fd1.response||[]).filter(f => FOOTBALL_LEAGUES.has(f.league?.id)),
        ];
        if (combined.length > 0) {
          combined.sort((a,b)=>new Date(a.fixture?.date)-new Date(b.fixture?.date));
          const hasLive = combined.some(f=>['1H','2H','ET','P','HT'].includes(f.fixture?.status?.short));
          return res.json({ success: true, source: hasLive?'live':'today', data: combined.map(transformFixture) });
        }
        return res.json({ success: false, error: 'No major league matches found' });
      }
      case 'football-fixtures': {
        // Parallel: yesterday + today + tomorrow = 3 calls max
        const [fr0, fr1, fr2] = await Promise.all([
          get(FB_BASE + '/fixtures', { date: todayStr(-1), timezone: 'UTC' }),
          get(FB_BASE + '/fixtures', { date: todayStr(0),  timezone: 'UTC' }),
          get(FB_BASE + '/fixtures', { date: todayStr(1),  timezone: 'UTC' }),
        ]);
        const all = [
          ...(fr0.response||[]).filter(f => FOOTBALL_LEAGUES.has(f.league?.id)),
          ...(fr1.response||[]).filter(f => FOOTBALL_LEAGUES.has(f.league?.id)),
          ...(fr2.response||[]).filter(f => FOOTBALL_LEAGUES.has(f.league?.id)),
        ];
      ...
      case 'basketball-live': {
        ...
        // Parallel: live + today + tomorrow simultaneously
        const [liveResp, bd0, bd1] = await Promise.all([
          bbGet('/games', { live: 'all' }),
        ...
        if (allGames.length > 0) {
          ...
          return res.json({ success: true, source: 'today', data: allGames.map(transformGame) });
        }
        return res.json({ success: false, error: 'No major basketball games found' });
      }
      case 'basketball-fixtures': {
        // Parallel fetch today + tomorrow — fast, no timeout risk
        const [bf0, bf1] = await Promise.all([
          bbGet('/games', { date: todayStr(0), timezone: 'UTC' }),
          bbGet('/games', { date: todayStr(1), timezone: 'UTC' }),
        ]);
        const all = [
          ...(bf0.response||[]).filter(g => BASKETBALL_LEAGUES.has(g.league?.id)),
          ...(bf1.response||[]).filter(g => BASKETBALL_LEAGUES.has(g.league?.id)),
        ];
