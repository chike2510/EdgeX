    ...
    switch (type) {
      ...
      case 'football-live': {
        ...
        if (requestedDate) {
          ...
          // Try major leagues first, fall back to all leagues so past dates always show something
          let filtered = all.filter(f => FOOTBALL_LEAGUES.has(f.league?.id));
          if (filtered.length === 0) filtered = all; // fallback: show everything on that date
          if (filtered.length > 0) {
            ...
            return res.json({ success: true, source: 'dated', date: requestedDate, data: filtered.slice(0,60).map(transformFixture) });
          }
      ...
      case 'football-fixtures': {
        const all = [];
        // Past 7 + next 7 days so date navigation works in both directions
        for (let i = -7; i <= 7; i++) {
        ...
        if (all.length > 0) return res.json({ success: true, data: all.slice(0,120).map(transformFixture) });
      ...
      case 'basketball-live': {
        ...
        if (requestedDate) {
          ...
          if (filtered.length === 0) filtered = all; // fallback: show all leagues on that date
...
// Basketball-specific helper — uses BB_BASE
