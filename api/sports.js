...
const FOOTBALL_LEAGUES = new Set([
  ...
  // More leagues
  197,  // Saudi Pro League
  292,  // J1 League
  188,  // China Super League
  218,  // Bundesliga 2
  40,   // Championship
  98,   // J-League
  253,  // Major League Soccer (US Open Cup)
  556,  // AFC Champions League
  529,  // Super Cup (various)
]);
...
// ── Server-side cache — avoids repeat calls within TTL ────────────────────────
const _cache = new Map();
function cached(key, ttlSeconds, fn) {
  const now = Date.now();
  const hit = _cache.get(key);
  if (hit && now - hit.ts < ttlSeconds * 1000) return Promise.resolve(hit.val);
    ...
    switch (type) {
      ...
      case 'football-live': {
        ...
        if (requestedDate) {
          ...
          const all = dated.response || [];
          // Try major leagues first, fall back to everything on that date
          let filtered = all.filter(f => FOOTBALL_LEAGUES.has(f.league?.id));
          ...
          if (filtered.length > 0) {
            ...
            return res.json({ success: true, source: 'dated', date: requestedDate, data: filtered.slice(0,80).map(transformFixture) });
          }
          return res.json({ success: false, error: 'No matches found for ' + requestedDate });
        }

        // Check live first (60s cache), then today (3min cache), then next 2 days in parallel
        const liveData = await cached('fb_live', 60, () => get(FB_BASE + '/fixtures', { live: 'all' }));
        const liveGames = (liveData.response || []).filter(f => FOOTBALL_LEAGUES.has(f.league?.id));
        if (liveGames.length > 0) {
          return res.json({ success: true, source: 'live', data: liveGames.map(transformFixture) });
        }
        // Today + next 2 days in parallel (3 calls), cached 3 min each
        const [d0, d1, d2] = await Promise.all([
          cached('fb_day_0', 180, () => get(FB_BASE + '/fixtures', { date: todayStr(0), timezone: 'UTC' })),
          cached('fb_day_1', 180, () => get(FB_BASE + '/fixtures', { date: todayStr(1), timezone: 'UTC' })),
          cached('fb_day_2', 180, () => get(FB_BASE + '/fixtures', { date: todayStr(2), timezone: 'UTC' })),
        ]);
        const combined = [
          ...(d0.response||[]), ...(d1.response||[]), ...(d2.response||[])
        ].filter(f => FOOTBALL_LEAGUES.has(f.league?.id));
        if (combined.length > 0) {
          combined.sort((a,b)=>new Date(a.fixture?.date)-new Date(b.fixture?.date));
          const liveCount = combined.filter(f=>['1H','2H','ET','P','HT'].includes(f.fixture?.status?.short)).length;
          return res.json({ success: true, source: liveCount>0?'live':'today', data: combined.map(transformFixture) });
        }
        return res.json({ success: false, error: 'No major league matches found' });
      }
      case 'football-fixtures': {
        // Fetch today + tomorrow in parallel, cached 3 min
        const [r1, r2] = await Promise.all([
          cached('fb_day_0', 180, () => get(FB_BASE + '/fixtures', { date: todayStr(0), timezone: 'UTC' })),
          cached('fb_day_1', 180, () => get(FB_BASE + '/fixtures', { date: todayStr(1), timezone: 'UTC' })),
        ]);
        const all = [
          ...(r1.response||[]).filter(f => FOOTBALL_LEAGUES.has(f.league?.id)),
          ...(r2.response||[]).filter(f => FOOTBALL_LEAGUES.has(f.league?.id)),
        ];
        ...
        if (all.length > 0) return res.json({ success: true, data: all.slice(0,100).map(transformFixture) });
        return res.json({ success: false, error: 'No fixtures found' });
      }

      // ── FULL FIXTURE DETAIL (events + lineups + stats in one call for FT matches)
      case 'football-fixture-detail': {
        if (!id) return res.json({ success: false, error: 'Missing id' });
        // Fetch fixture with events included
        const fixture = await get(FB_BASE + '/fixtures', { id, timezone: 'UTC' });
        const fix = fixture.response?.[0];
        if (!fix) return res.json({ success: false, error: 'Fixture not found' });
        return res.json({
          success: true,
          events:  (fix.events||[]),
          score:   fix.score,
      ...
      case 'basketball-live': {
        ...
        if (requestedDate) {
          ...
          const all = dated.response || [];
          let filtered = all.filter(g => BASKETBALL_LEAGUES.has(g.league?.id));
          if (filtered.length === 0) filtered = all; // show all leagues if no major ones
          if (filtered.length > 0) return res.json({ success: true, source: 'dated', data: filtered.map(transformGame) });
          return res.json({ success: false, error: 'No games found for ' + requestedDate });
        }

        const liveResp = await cached('bb_live', 60, () => bbGet('/games', { live: 'all' }));
        ...
        // Today + tomorrow in parallel, cached 3 min
        const [bd0, bd1] = await Promise.all([
          cached('bb_day_0', 180, () => bbGet('/games', { date: todayStr(0), timezone: 'UTC' })),
          cached('bb_day_1', 180, () => bbGet('/games', { date: todayStr(1), timezone: 'UTC' })),
        ]);
        const bbGames = [
          ...(bd0.response||[]).filter(g => BASKETBALL_LEAGUES.has(g.league?.id)),
          ...(bd1.response||[]).filter(g => BASKETBALL_LEAGUES.has(g.league?.id)),
        ];
        if (bbGames.length > 0) return res.json({ success: true, source: 'today', data: bbGames.map(transformGame) });
        return res.json({ success: false, error: 'No major basketball games found' });
      }
      case 'basketball-fixtures': {
        // Parallel fetch today + tomorrow
        const [b1, b2] = await Promise.all([
          bbGet('/games', { date: todayStr(0), timezone: 'UTC' }),
          bbGet('/games', { date: todayStr(1), timezone: 'UTC' }),
        ]);
        const all = [
          ...(b1.response||[]).filter(g => BASKETBALL_LEAGUES.has(g.league?.id)),
          ...(b2.response||[]).filter(g => BASKETBALL_LEAGUES.has(g.league?.id)),
        ];
        if (all.length > 0) return res.json({ success: true, data: all.slice(0,60).map(transformGame) });
        return res.json({ success: false, error: 'No fixtures found' });
      }
      ...
      case 'football-standings': {
        ...
        const ckey = `std_${league}_${season||2025}`;
        const data = await cached(ckey, 1800, () => get(FB_BASE + '/standings', { league: league||39, season: season||2025 }));
      ...
      case 'football-topscorers': {
        const { league, season } = req.query;
        const data = await cached(`ts_${league}_${season||2025}`, 1800, () => get(FB_BASE + '/players/topscorers', { league: league||39, season: season||2025 }));
      ...
      case 'football-topassists': {
        const { league, season } = req.query;
        const data = await cached(`ta_${league}_${season||2025}`, 1800, () => get(FB_BASE + '/players/topassists', { league: league||39, season: season||2025 }));
        return res.json({ success: true, data: (data.response||[]).slice(0,20) });
      }
      ...
      // ── HEAD TO HEAD ────────────────────────────────────────────────────────
      case 'football-h2h': {
        const { h2h, home, away } = req.query;
        // h2h can be "teamId1-teamId2" directly, or we search by name
        let h2hParam = h2h;
        if (!h2hParam && home && away) {
          // Search both teams by name to get their IDs
          const [hD, aD] = await Promise.all([
            get(FB_BASE + '/teams', { search: home }),
            get(FB_BASE + '/teams', { search: away }),
          ]);
          const hId = hD.response?.[0]?.team?.id;
          const aId = aD.response?.[0]?.team?.id;
          if (!hId || !aId) return res.json({ success: false, error: 'Teams not found' });
          h2hParam = hId + '-' + aId;
        }
        if (!h2hParam) return res.json({ success: false, error: 'Missing h2h or home/away params' });
        const data = await get(FB_BASE + '/fixtures/headtohead', { h2h: h2hParam, last: 10 });
        return res.json({ success: true, data: (data.response||[]), h2h: h2hParam });
      }
...
// Basketball-specific helper
function bbGet(path, params={}) {
  return get(BB_BASE + path, params);
}
