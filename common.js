...
function injectNav() {
  ...
  // ── Mobile: inject overlay + hamburger only (bottom nav removed) ─────────
...
function injectTicker() {
  ...
  const items = [
    ...
    {sym:'DOGE', val:'$0.182',  chg:'+3.1%', pos:true},
    ...
    {sym:'S&P',  val:'5,248',   chg:'+0.6%', pos:true},
    {sym:'NASDAQ',val:'16,340', chg:'+0.8%', pos:true},
  ];
  const html = items.map(i => `<div class="ticker-item">
    <span class="t-sym">${i.sym}</span>
    <span class="t-val">${i.val}</span>
    <span class="t-chg ${i.pos?'pos':'neg'}">${i.pos?'▲':'▼'} ${i.chg}</span>
