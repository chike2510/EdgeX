(() => {
  ...
  function card(item) {
    ...
    return `<article class="edgex-v2-card edgex-player-card">
      ...
      <div class="edgex-player-card-grid"><div><small>Market</small><strong>${esc(item.displayName || item.marketType || 'Market unavailable')}</strong></div><div><small>Line</small><strong>${line === null ? unavailable('Unavailable') : esc(line)}</strong></div><div><small>Fixture</small><strong>${item.fixtureTime ? esc(new Date(item.fixtureTime).toLocaleString()) : unavailable('Unavailable')}</strong></div></div>
      <div class="edgex-v2-state edgex-player-evidence"><strong>AI verdict</strong><span>Waiting for enough provider-backed evidence</span><small>Player Edge does not invent projections or confidence when historical inputs are unavailable.</small></div><button class="edgex-v2-button player-analysis-button" data-player-id="${esc(item.id || item.playerId || item.displayName || item.playerName)}" type="button">View analysis</button>
    </article>`;
  }
  ...
  function render() {
    ...
    document.querySelectorAll('.player-analysis-button').forEach(button => button.addEventListener('click', () => { const item = state.props.find(candidate => String(candidate.id || candidate.playerId || candidate.displayName || candidate.playerName) === button.dataset.playerId); if (item) showAnalysis(item); }));
  }

  async function showAnalysis(item) { const dialog = document.createElement('dialog'); dialog.className = 'edgex-v2-dialog'; dialog.innerHTML = `<div class="edgex-v2-card"><button class="edgex-v2-dialog-close" type="button">Close</button><span class="edgex-v2-eyebrow">Player Edge analysis</span><h2>${esc(item.playerName || item.displayName || 'Player unavailable')}</h2><p>${esc(item.team || 'Team unavailable')} ${item.opponent ? `vs ${esc(item.opponent)}` : ''} · ${esc(item.displayName || item.marketType || 'Market unavailable')}</p><div class="edgex-market-stats"><div><small>Provider line</small><strong>${item.line == null ? 'Unavailable' : esc(item.line)}</strong></div><div><small>Projection</small><strong id="player-ai-projection">Loading…</strong></div><div><small>Confidence</small><strong id="player-ai-confidence">Loading…</strong></div></div><div class="edgex-v2-state" id="player-ai-state"><strong>Generating a structured read</strong><span>EdgeX will use only the provider-backed player context supplied here.</span></div><div id="player-ai-detail"></div></div>`; document.body.appendChild(dialog); dialog.querySelector('.edgex-v2-dialog-close').onclick = () => dialog.remove(); dialog.showModal(); try { const response = await fetch('/api/edge-analysis', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ domain: 'player-edge', subject: item.playerName || item.displayName || 'Player', data: { id: item.id, playerId: item.playerId, playerName: item.playerName, team: item.team, opponent: item.opponent, sport: item.sport, marketType: item.marketType, displayName: item.displayName, line: item.line, status: item.status, fixtureTime: item.fixtureTime, provider: item.provider, providerData: item } }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || 'Analysis unavailable'); dialog.querySelector('#player-ai-projection').textContent = result.projection || 'Unavailable'; dialog.querySelector('#player-ai-confidence').textContent = result.confidence == null ? 'Unavailable' : `${result.confidence}%`; dialog.querySelector('#player-ai-state').innerHTML = `<strong>${esc(result.verdict || 'Analysis returned')}</strong><span>${esc(result.dataQuality || 'Data quality unavailable')}</span>`; dialog.querySelector('#player-ai-detail').innerHTML = `<div class="edgex-v2-state"><strong>Why this read</strong><span>${esc((result.reasons || []).join(' ') || 'No supporting reasons returned.')}</span><small>${esc((result.uncertainties || []).join(' ') || 'No additional uncertainty returned.')}</small></div>`; } catch (error) { dialog.querySelector('#player-ai-state').innerHTML = `<strong>Analysis unavailable</strong><span>${esc(error.message || 'The structured analysis service could not respond.')}</span>`; dialog.querySelector('#player-ai-projection').textContent = 'Unavailable'; dialog.querySelector('#player-ai-confidence').textContent = 'Unavailable'; } }
