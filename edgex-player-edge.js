(() => {
  ...
  function card(item) {
    ...
    return `<article class="edgex-v2-card edgex-player-card">
      ...
      <div class="edgex-v2-state edgex-player-evidence"><strong>${item.selection && line !== null ? 'Provider context ready' : 'Provider context incomplete'}</strong><span>${item.selection && line !== null ? 'The market option and line came directly from Squads.' : 'Squads returned this record without a complete market option or line.'}</span><small>AI projection and confidence appear only after a structured analysis request; unavailable AI output does not replace the provider data.</small></div><button class="edgex-v2-button player-analysis-button" data-player-id="${esc(item.id || item.playerId || item.displayName || item.playerName)}" type="button">View analysis</button>
    </article>`;
  }
